...
function read(){
    ...
    // select all query
     ...
     $query = "SELECT
                c.category_name as category_name, p.art_id,  p.art_title,p.art_thumb,p.art_sum,p.created,p.category_id
            FROM
                " . $this->table_name . " p
                LEFT JOIN
                    category c
            ...
            FROM
                ...
                LEFT JOIN
                    category c
                        ON p.category_id = c.category_id
            WHERE
                 p.art_title LIKE ? OR p.art_sum LIKE ? OR c.category_name LIKE ?            
...
function readOne(){
    ...
    $this->art_thumb= $row['art_thumb'];
    $this->art_sum= $row['art_sum'];
...
function readcontent(){
    ...
    // query to read single record
    $query = "SELECT
                 arcon_id,art_id, arcon_num,arcon_type,arcon_body
            FROM
                " . $this->table_name2 . " 
